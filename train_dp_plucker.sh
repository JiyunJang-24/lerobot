...
CUDA_VISIBLE_DEVICES=2 python lerobot/scripts/train.py \
  ...
  --job_name=DP_plucker_ex1_angle_from_0_22.5_45_315_338.5 \
  --use_plucker=true \
  --use_dynamics_basis=false &

CUDA_VISIBLE_DEVICES=3 python lerobot/scripts/train.py \
  --dataset.repo_id=[cam_info_10_10_0.0_0.0/v-1.000-1.000_num1,cam_info_10_10_0.0_0.0/v-1.000-1.000_num5] \
  --dataset.root=/data1/local/shortcut-learning-in-grps/dataset_git/libero_spatial_no_noops_island_1_lerobot \
  --dataset.image_transforms.enable=false \
  --dataset.use_imagenet_stats=false \
  --dataset.split_episodes=false \
  ...
  --wandb.disable_artifact=true \
  --wandb.entity=DynamicVLA \
  --job_name=DP_plucker_ex1_angle_from_0 \
  --use_plucker=true \
  --use_dynamics_basis=false
