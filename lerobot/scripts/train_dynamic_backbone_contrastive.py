...
def supervised_contrastive_loss(z: torch.Tensor, y: torch.Tensor, temperature: float = 0.07) -> torch.Tensor:
    ...
    # 유사도 행렬 (N, N)
    sim = z @ z.t()  # cosine 유사도 (정규화 했으니 내적 = 코사인)
    sim = sim / temperature
    ...
    # 같은 클래스(positive) 마스크 (자기 자신 제외)
    y = y.view(-1, 1)
    pos_mask = torch.eq(y, y.t()) & (~self_mask)  # (N,N) boolean

    # 분모: i행에서 자기 자신 제외한 전체 exp(sim)
    # 수치안정화: 행별 max 빼주기
    sim_max, _ = sim.max(dim=1, keepdim=True)
    sim_exp = torch.exp(sim - sim_max)  # (N,N)

    # 분모에서 자기 자신은 제외
    denom = (sim_exp * (~self_mask)).sum(dim=1, keepdim=True) + 1e-12  # (N,1)

    # 분자: positive들만 합
    pos_exp = sim_exp * pos_mask  # (N,N)
    pos_count = pos_mask.sum(dim=1)  # 각 앵커별 양성 개수 (N,)

    # 양성이 하나도 없는 앵커는 손실에서 제외(해당 배치에 같은 클래스 샘플이 1개뿐일 때)
    valid = pos_count > 0
    # 각 앵커별 평균 log-softmax(-) :  - (1/|P(i)|) sum_{p in P(i)} log( exp(sim_i,p)/sum_{a≠i} exp(sim_i,a) )
    log_prob = torch.log((pos_exp.sum(dim=1, keepdim=True) + 1e-12) / denom)  # (N,1)
    loss_per_i = -(log_prob.squeeze(1)) / (pos_count + 1e-12)  # (N,)

    if valid.any():
        return loss_per_i[valid].mean()
    else:
        # 배치에 모든 클래스가 전부 유니크면(양성이 하나도 없으면) 손실 0 반환
...
def train(cfg: TrainPipelineConfig):    
    ...
    model.to(device)
    # ▶ contrastive projection head (64 -> 128)
    ...
    proj_head.to(device)
    learnable_params = [p for p in model.parameters() if p.requires_grad] + list(proj_head.parameters())

    # learnable_params = [p for p in model.parameters() if p.requires_grad]
    optimizer = torch.optim.AdamW(
        learnable_params,
        lr=getattr(cfg.optimizer, "lr", 3e-4),
    ...
    contrastive_temperature = getattr(cfg.policy, "contrastive_temperature", 0.07)
    ...
    if cfg.use_depth:
        ...
        depth_backbone.load_state_dict(torch.load(f'/Depth_Anything_V2/checkpoints/depth_anything_v2_{encoder}.pth'))
    ...
    for _ in range(step, cfg.steps):
        ...
        model.optical_backbone.eval()
        with torch.autocast(device_type=device.type) if use_amp else nullcontext():
            dynamic_feature = model(img0_224, img1_224, action=action)  # (B, 64)
            z = proj_head(dynamic_feature)                      # (B, 128)
            loss = supervised_contrastive_loss(
                z, target, temperature=contrastive_temperature
            )
        ...
        grad_norm = torch.nn.utils.clip_grad_norm_(learnable_params, cfg.optimizer.grad_clip_norm, error_if_nonfinite=False)
